...
class ChildInterest(Base):
    ...
    enthusiasm_level = Column(SQLEnum(EnthusiasmLevel), default=EnthusiasmLevel.LIKES)
    notes = Column(Text)
...
class ChildRoutine(Base):
    ...
    potty_training_stage = Column(SQLEnum(ToiletStage), default=ToiletStage.NOT_STARTED)
...
class ChildDevelopment(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    child_id = Column(UUID(as_uuid=True), ForeignKey("children.id", ondelete="CASCADE"), nullable=False, unique=True)
    walking_stage = Column(SQLEnum(WalkingStage), default=WalkingStage.NOT_WALKING)
    talking_stage = Column(SQLEnum(TalkingStage), default=TalkingStage.BABBLING)
    feeding_stage = Column(SQLEnum(FeedingStage), default=FeedingStage.MILK_ONLY)
    toilet_stage = Column(SQLEnum(ToiletStage), default=ToiletStage.NOT_STARTED)
