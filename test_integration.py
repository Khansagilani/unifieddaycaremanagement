...
class NestCareTestSuite:
    ...
    def login(self, role="admin"):
        ...
        self.test(f"Login successful (HTTP {response.status_code})", response.status_code == 200)
        ...
        if response.status_code == 200:
            ...
            if me_response.status_code == 200:
                ...
                print(f"  Current user: {self.user.get('email')} ({self.user.get('role')})")
            return True
        return False
    ...
    def test_payment_flow(self):
        ...
        self.test(f"Get invoices (HTTP {response.status_code})", response.status_code == 200)
        
        invoices = response.json().get("data", [])
        self.test(f"Invoices found", len(invoices) > 0, f"- Found {len(invoices)} invoices")
        
        if invoices:
            invoice = invoices[0]
            invoice_id = invoice.get("id")
            print(f"  Invoice #{invoice.get('invoice_number')}: ${invoice.get('amount_cents')/100:.2f}")
            ...
            self.test(f"Create payment intent (HTTP {response.status_code})", response.status_code == 200)
    ...
    def test_staff_pages_data(self):
        ...
        self.test(f"Get children (HTTP {response.status_code})", response.status_code == 200)
        children = response.json().get("data", [])
        self.test(f"Children found", len(children) > 0, f"- Found {len(children)} children")
        
        # 2. Get daily logs (for dashboard)
        print(f"\n→ Fetching daily logs...")
        response = self.session.get(f"{BASE_URL}/api/health-daily/daily-logs")
        self.test(f"Get daily logs (HTTP {response.status_code})", response.status_code == 200)
        ...
        self.test(f"Get attendance (HTTP {response.status_code})", response.status_code in [200, 404])
        ...
        if children:
            ...
            self.test(f"Check-in endpoint (HTTP {response.status_code})", response.status_code in [200, 201, 400, 409])
        ...
        if children:
            ...
            self.test(f"Create daily log (HTTP {response.status_code})", response.status_code in [200, 201, 400])
            
        # 6. Get conversations
        print(f"\n→ Fetching conversations...")
        response = self.session.get(f"{BASE_URL}/api/media/conversations")
        self.test(f"Get conversations (HTTP {response.status_code})", response.status_code == 200)
    ...
    def test_auth_and_refresh(self):
        ...
        self.test(f"Get /me (HTTP {response.status_code})", response.status_code == 200)
